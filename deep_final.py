...
def t_batch(netwrk,method_opti, rule, size_batch, device = 'cpu'):
    ...
    for j in range(size_batch):
        ...
        for pointr, opt in enumerate(yields):
            ...
            privation_total = privation_total + privation
    method_opti.step()
    return privation_total/size_batch
...
def function_sweep():
    ...
    Loss = fr_training(model, learning_rate = wandb.config.learning_rate,batches_number = 2000, size_batch = wandb.config.size_batch)
...
os.environ['WANDB_NOTEBOOK_NAME'] = 'Deep_final.ipynb'
wandb.login()
sweep_id = wandb.sweep(sweep_config, entity="dl_sumit", project="Deep_Learning_Assignment_3")
